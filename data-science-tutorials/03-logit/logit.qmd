...
In the last lecture, we focused on data wrangling (import, tidy, transform, visualize). Now we progress to the modeling part. In this lecture, we focus on predictive modeling using machine learning methods (logistic, random forest, ...). In the next lecture, we focus on policy evaluation using double machine learning.
...
The logistic regression model has an AUC of `r round(auc(roc_data))`, which indicates that the model has a good discrimination ability. However, if we want to evaluate the model's predictive performance, simply fitting models and calculating AUC is not enough.
