...
-   We've done tremendous work carrying out an intervention program. You want to demonstrate to stakeholders (government, funding agencies, etc) that your program "works."

    -   We compared the average of [Food Security Rasch Scale Score in the past 30 days](https://www.ers.usda.gov/media/8282/short2012.pdf) (higher score indicates lower food security among program) among program participants versus those not participating our program. We found a significant difference in the averages 😄 But quickly people criticize our analysis as biased 🙁 What could go wrong?

    -   In another try, we ran a multiple linear regression that included the program participation status and many socio-economical variables (race, age, income, region, ...). Then we found that program participation does not significantly improve the food security Rasch score 🙁
...
2. The policy variable $D$ can be multi-dimensional (multi-level intervention: food pantry, education classes, ads.).
